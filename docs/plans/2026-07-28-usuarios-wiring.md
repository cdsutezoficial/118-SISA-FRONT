...
5. **`ROLE_OPTIONS`/`DIVISION_SCOPED_ROLES` se consolidaron en `shared/identity/roles.ts`** pero `UsuariosList.tsx` mantiene su copia local — es la única pantalla que no necesita el scope de división, y tocarla estaba fuera del alcance mínimo de este cambio. **Actualización 2026-09-08**: esta nota quedó obsoleta — `UsuariosList.tsx` se migró por completo a los componentes compartidos de listado (`core/components/list.tsx`: `DataTable`, `MobileCards`, `Pagination`, `FilterBar`, `Breadcrumb`, `PageHeader`, etc.), integrando `ROLE_LABELS`/`ROLE_BADGE_STYLE` locales como catálogo de la pantalla. Detalle de la migración en el resumen de la sesión; en particular se eliminó el botón mock "Cambiar estado" (el backend NO tiene `PATCH /users/{id}/status` — el estatus del `User` es 3 vías ACTIVE/INACTIVE/LOCKED y solo muta por login/lock/unlock) y la columna Estado pasó a `type: 'badge'` con `ESTADO_BADGE_MAP`.
6. **`UsuariosList.tsx` (migración 2026-09-08)**: acciones de fila alineadas con el resto de listados — el "Ver" usa `actions.view` del `DataTable`, y quedan solo botones contextuales en `extra` (Restablecer contraseña, Desbloquear solo si `LOCKED`).

**Verificación**: `npx tsc --noEmit` desde `118-SISA-FRONT/` — limpio, sin errores.

**Ejecución (2026-09-08)**: migración de `UsuariosList.tsx` a componentes compartidos commiteada junto con este plan. `estado` del commit: `feat(usuarios): migrar UsuariosList a componentes compartidos`.
