...
- [x] 2.16 Screen 16 Portal Acceso — `pages/portal/PortalInduccion.tsx`, route `/portal/induccion`; simulated LlaveMX + folio/CURP, sets role `CANDIDATO`
- [x] 2.17 Screen 17 Portal Pago Inducción — `pages/portal/PortalInduccionPago.tsx`, route `/portal/induccion/pago`; already-paid alt state, simulated overlay (depends on 2.16)
...
- [x] 3.6 CRITICAL remediation — route-level role guard: added `src/app/shared/RequireRole.tsx`, wrapped all 14 role-restricted `/admision/*` routes in `router.tsx` (allowed roles per `03-admision.md`'s "Rol activo en sidebar"), and wired `usePendingToast()`/`Toast` into `AdmisionDashboard.tsx` so the redirect toast renders. Deliberately left the `/admision` index route unguarded — see `specs/mock-role-context/spec.md`'s "Deliberate exception" note for why (redirect-target loop risk). `npm run typecheck` and `npm run build` both pass. Branch: `admision/fix-route-role-guard` (off `admision/foundation-a`).
